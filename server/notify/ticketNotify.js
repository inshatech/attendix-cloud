 ...
 * Light default + dark-mode CSS override + mobile-responsive.
 */
...
<head>
  ...
  <style>
    ...
    @media (prefers-color-scheme:dark) {
      ...
      .tc    { background:#0f0f1c !important; border-color:#1e1e30 !important; }
      .mc    { background:#0a0a14 !important; border-color:#1e1e30 !important; }
      .c-head { color:#e0e0f0 !important; }
      .c-sub  { color:#5050a0 !important; }
      .c-body { color:#9090b8 !important; }
  ...
  <div class="aw" style="max-width:540px;margin:32px auto;padding:0 12px;">
    ...
    <div class="ah" style="background:#ffffff;border:1px solid #dde0f0;border-radius:14px 14px 0 0;padding:18px 24px;display:flex;align-items:center;gap:12px;">
      ...
      <div>
        ...
        <p class="c-sub" style="margin:2px 0 0;color:#5050a0;font-size:0.65rem;font-family:monospace;">${tagline}</p>
      </div>
    </div>

    <!-- Body -->
    <div class="ab" style="background:#ffffff;border:1px solid #dde0f0;border-top:none;padding:26px 24px;">
      ${body}
    </div>
...
function ticketCard(ticket) {
  ...
  return `
  <div class="tc" style="background:#f0f2ff;border:1px solid #dde0f0;border-radius:8px;padding:14px 16px;margin:16px 0;border-left:3px solid ${pc};">
    <p style="margin:0 0 4px;color:#9090b0;font-size:0.65rem;font-family:monospace;">${ticket.ticketId}</p>
    <p class="c-head" style="margin:0 0 8px;color:#1a1a2e;font-weight:600;font-size:0.875rem;">${ticket.subject}</p>
    <div style="display:flex;gap:6px;flex-wrap:wrap;">
      ...
      <span style="color:#5050a0;background:#e8eaff;padding:2px 8px;border-radius:99px;font-size:0.65rem;text-transform:capitalize;">${ticket.category||'general'}</span>
      <span style="color:#5050a0;background:#e8eaff;padding:2px 8px;border-radius:99px;font-size:0.65rem;">From: ${ticket.userName||''}</span>
    </div>
  </div>`;
}
...
function msgBox(message) {
  return `
  <div class="mc" style="background:#f4f4fc;border:1px solid #dde0f0;border-radius:8px;padding:14px 16px;margin:12px 0;">
    <p style="margin:0 0 6px;color:#9090b0;font-size:0.68rem;">${message.authorName||''} · ${new Date(message.createdAt||Date.now()).toLocaleString('en-IN')}</p>
    <p class="c-body" style="margin:0;color:#4a4a80;font-size:0.85rem;line-height:1.65;white-space:pre-wrap;">${String(message.body||'').slice(0,600)}</p>
  </div>`;
}
...
async function notifyNewTicket(ticket) {
  fire(async () => {
    ...
    const html = wrap(`
      <h2 style="color:#1a1a2e;margin:0 0 4px;font-size:1.15rem;font-weight:800;letter-spacing:-0.02em;">🎫 New Support Ticket</h2>
      <p style="color:#5050a0;font-size:0.82rem;margin:0 0 20px;">A new ticket has been submitted and needs attention.</p>
      ${ticketCard(ticket)}
      ${ticket.body ? msgBox({ authorName: ticket.userName, body: ticket.body }) : ''}
      <p style="color:#9090b0;font-size:0.8rem;margin:16px 0 0;">Log in to the admin panel to assign and respond to this ticket.</p>
    `, brand);
...
async function notifyStaffReply(ticket, message) {
  ...
  fire(async () => {
    ...
    const html = wrap(`
      <h2 style="color:#1a1a2e;margin:0 0 4px;font-size:1.15rem;font-weight:800;letter-spacing:-0.02em;">💬 Reply on Your Support Ticket</h2>
      <p style="color:#5050a0;font-size:0.82rem;margin:0 0 20px;">The support team has responded to your ticket.</p>
      ${ticketCard(ticket)}
      ${msgBox(message)}
      <div style="background:rgba(88,166,255,0.07);border:1px solid rgba(88,166,255,0.2);border-radius:8px;padding:12px 16px;margin-top:16px;">
        <p style="margin:0;color:#5060a0;font-size:0.8rem;">Log in to view the full conversation and send a reply.</p>
      </div>
    `, brand);
...
async function notifyUserReply(ticket, message) {
  fire(async () => {
    ...
    const html = wrap(`
      <h2 style="color:#1a1a2e;margin:0 0 4px;font-size:1.15rem;font-weight:800;letter-spacing:-0.02em;">↩️ User Replied on Ticket</h2>
      <p style="color:#5050a0;font-size:0.82rem;margin:0 0 20px;">The customer has added a reply and may need a response.</p>
      ${ticketCard(ticket)}
      ${msgBox(message)}
    `, brand);
    ...
    const statusNote = ticket.status === 'resolved'
      ...
      : ticket.status === 'waiting'
        ? `<div style="background:rgba(250,204,21,0.08);border:1px solid rgba(250,204,21,0.25);border-radius:8px;padding:14px 16px;margin-top:16px;">
             <p style="margin:0;color:#d4a017;font-size:0.85rem;line-height:1.6;">⏳ Our team needs more information from you. Please log in and reply to this ticket.</p>
           </div>`
        : '';

    const html = wrap(`
      <h2 style="color:#1a1a2e;margin:0 0 4px;font-size:1.15rem;font-weight:800;letter-spacing:-0.02em;">${label}</h2>
      <p style="color:#5050a0;font-size:0.82rem;margin:0 0 20px;">Your support ticket status has been updated.</p>
      ${ticketCard(ticket)}
      ${statusNote}
    `, brand);
...
async function notifyTicketCreatedForUser(ticket) {
  fire(async () => {
    ...
    const html = wrap(`
      <h2 style="color:#1a1a2e;margin:0 0 4px;font-size:1.15rem;font-weight:800;letter-spacing:-0.02em;">🎫 Support Ticket Opened for You</h2>
      <p style="color:#5050a0;font-size:0.82rem;margin:0 0 20px;">Our support team has opened a ticket on your behalf.</p>
      ${ticketCard(ticket)}
      <div style="background:rgba(88,166,255,0.07);border:1px solid rgba(88,166,255,0.2);border-radius:8px;padding:12px 16px;margin-top:16px;">
        <p style="margin:0;color:#5060a0;font-size:0.8rem;">Log in to view and respond to this ticket.</p>
      </div>
    `, brand);
